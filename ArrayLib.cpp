...
int getShuffledNumber(int* array, int size, int *numLinesRun) {
    (*numLinesRun)+=4;
    int randomNum = genRandInt(1, size, numLinesRun);
    if (find(array, size, randomNum) != -1) {
        (*numLinesRun)++;
        return getShuffledNumber(array, size, numLinesRun);
    }
    return randomNum;
}
...
int *genShuffledArray(int size, int *numLinesRun) {
    ...
    (*numLinesRun)+=2;
    for (int i = 0; i < size; i++) {
        shuffledArray[i] = getShuffledNumber(shuffledArray, size, numLinesRun);
        (*numLinesRun)+=3;
    }
    (*numLinesRun)++;
